...
| DATA-20260819-001 | 데이터 | secrets 주입 전 beta 신선도 스톱갭 | Reactive | IMPLEMENTED | P2 | 2026-08-19 사용자 결정: (b) 임시 재게시 자동화. `daily-batch.yml`에 stopgap 스텝 추가 — READY 아닌 동안 `db:seed`(deterministic mock)로 `batch_state` 재게시, DATA-003 재개 시 자동 비활성(ADR-005). **BLOCKER: GitHub secrets에 실제 `DATABASE_URL` 주입 필요** — Vercel에서 Sensitive로 지정돼 CLI 복사 불가, 사용자가 대시보드 값으로 `gh secret set DATABASE_URL` 실행해야 함 |
