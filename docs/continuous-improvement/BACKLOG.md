...
| INT-20260821-001 | 내부 | 배포 번들에 readiness 정적 파일 포함 (outputFileTracingIncludes) | Progressive | NEW | P1 | 2026-08-21 발견: `*_available` 17개 영구 실패 원인. next.config에 ops 라우트 대상 정적 파일 포함 시 파일 기반 체크 약 14개 해제 예상(readiness 18→~32). partner 키 불필요 — 승인 후 구현·재배포 검증 |
