...
| DATA-20260820-001 | 데이터 | 운영 env 패키지 주입 | Reactive | IN_PROGRESS | P2 | 2026-08-20 1차 적용: SOURCE_*_ENABLED 6종 + OPS_READINESS_TOKEN(생성, Vercel·GH·.env.local) → readiness 16→**18**(+2). 잔여 사용자 값: SUPPORT_EMAIL, OPS_ALERT_WEBHOOK_URL(웹훅 생성 필요) — 주입 시 +4~6 추가 기대. SERVICE_REQUIRE_POSTGRES는 출시 시점 결정으로 제외 |
